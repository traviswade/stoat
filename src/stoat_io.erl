...
read(Io, Prompt) ->
	% ?p("reading io:", []),
    case request(Io, {get_until,unicode,Prompt,stoat_lex,tokens,[1]}) of
	{ok,Toks,_EndLine} ->
	    erl_parse:parse_term(Toks);
		% stoat_parse:toks(Toks);
...
read(Io, Prompt, Pos0, Options) ->
    Args = [Pos0,Options],
    case request(Io, {get_until,unicode,Prompt, stoat_lex,tokens,Args}) of
	{ok,Toks,EndLocation} ->
...
scan_erl_exprs(Io, Prompt, Pos0, Options) ->
	% ?p("stoat_io:scan_erl_exprs/4~n", []),
    request(Io, {get_until,unicode,Prompt, stoat_lex,tokens,[Pos0]}).
...
scan_erl_form(Io, Prompt, Pos0, Options) ->
    request(Io, {get_until,unicode,Prompt, stoat_lex,tokens,[Pos0,Options]}).
...
parse_erl_exprs(Io, Prompt, Pos0, Options) ->
    case request(Io, {get_until,unicode,Prompt, stoat_lex,tokens,[Pos0,Options]}) of
	{ok,Toks,EndPos} ->
	    case stoat_parse:parse_exprs(Toks) of
