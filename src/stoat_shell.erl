%
% Copy-paste fork of stdlib/shell.erl ~r17
%
%%
%% %CopyrightBegin%
%%
...
server(StartSync) ->
	...
	true ->
	    stoat_io:fwrite(<<"Stoat 0.1\n">>, []);
	_undefined_or_false ->		
		
	    stoat_io:fwrite(<<"Stoat 0.1  (abort with ^G)\n">>, [])
    end,
    erase(no_control_g),
...
do_io_scan (Group, Prompt, N) ->
	stoat_io:scan_erl_exprs(group_leader(), Prompt, N).

get_command(Prompt, Eval, Bs, RT, Ds) ->
	% p("getting command: ~p~n", [group_leader()]),
    Parse =
        fun() ->
			% p("parsing (~p)~n", [Prompt]),
                exit(
                  case
                      stoat_io:scan_erl_exprs(group_leader(), Prompt, 1)
						% do_io_scan(group_leader(), Prompt, 1)
                  of
                      {ok,Toks,_EndPos} ->
						  % p("got tokens: ~p~n", [Toks]),
                          stoat_parse:parse_exprs(Toks);
                      {eof,_EndPos} ->
							% p("got EOF: ~p~n", [eof]),
                          eof;
                      {error,ErrorInfo,_EndPos} ->
							 % p("got ERROR: ~p~n", [ErrorInfo]),
                          %% Skip the rest of the line:
                          Opts = stoat_io:getopts(),
                          TmpOpts = lists:keyreplace(echo, 1, Opts,
                                                     {echo, false}),
                          _ = stoat_io:setopts(TmpOpts),
                          _ = stoat_io:get_line(''),
                          _ = stoat_io:setopts(Opts),
                          {error,ErrorInfo};
                      Else ->
							  % p("got something ELSE: ~p~n", [Else]),
                          Else
                  end
                )
        end,
...
bad_prompt_func(M) ->
    fwrite_severity(benign, "Bad prompt function: ~p~n", []).
...
report_exception(Class, Severity, {Reason,Stacktrace}, RT) ->
    ...
    stoat_io:requests([{put_chars, latin1, Tag},
        ...
        ok ->
            ...
            if
                Es =:= [] ->
                    VS = pp(V0, 1, RT),
                    case W of
                        cmd -> stoat_io:requests([{put_chars, unicode, VS}, nl]);
                        pmt -> ok
                    end,
...
local_func(rp, [A], Bs0, _Shell, RT, Lf, Ef) ->
    ...
    Cs = io_lib_pretty:print(V, ([{column, 1},
                                 ...
                                 ++ enc())),
    stoat_io:requests([{put_chars, unicode, Cs}, nl]),
    {value,ok,Bs};
...
list_commands([{{N,command},Es0}, {{N,result}, V} |Ds], RT) ->
    ...
    I = iolist_size(Ns),
    stoat_io:requests([{put_chars, latin1, Ns},
...
list_bindings([{Name,Val}|Bs], RT) ->
    case erl_eval:fun_data(Val) of
        ...
        false ->
            ...
            ValS = pp(Val, Nl, RT),
            stoat_io:requests([{put_chars, latin1, Namel},
...
list_records(Records) ->
    lists:foreach(fun({_Name,Attr}) -> 
                          stoat_io:fwrite(<<"~ts">>, [erl_pp:attribute(Attr, enc())])
                  end, Records).
...
encoding() ->
    ...
    Encoding.
enc() ->
    case lists:keyfind(encoding, 1, stoat_io:getopts()) of
